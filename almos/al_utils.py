...
def write_log_header(logger, title, border="="):
    """
    Write a boxed section header to the AL log.
    """
    width = max(LOG_MIN_WIDTH, len(title) + 6)
    line = border * width
    logger.write(f"\n{line}")
...
def write_log_block(logger, title, rows):
    """
    Write a titled block with aligned bullet key/value rows.
    """
    logger.write(f"\no {title}")
    for label, value in rows:
        logger.write(f"   - {label}: {value}")


def format_objective_label(objective):
    """
    Convert internal objective names into user-facing text.
    """
    if objective is None or str(objective).strip().lower() in {"", "none"}:
        return "not used"
    if str(objective).lower() == "max":
        return "maximize (higher values are better)"
    return "minimize (lower values are better)"


def format_strategy_label(strategy_name):
    """
    Convert internal strategy names into user-facing text.
    """
    if strategy_name == "model":
        return "model improvement (highest uncertainty)"
    return "hit discovery (prediction weighted by uncertainty)"


def format_strategy_reason(strategy):
    """
    Explain why a specific acquisition strategy was selected.
    """
    if strategy["score_source"] == "manual_override":
        if strategy["strategy"] == "model":
            return "manual mode forced uncertainty-based selection"
        return "manual mode forced hit discovery"

    if strategy["strategy"] == "model":
        if strategy["score"] is None:
            return "model score could not be read, so uncertainty was prioritized"
        return (
            f"model score {strategy['score']} is 6 or lower, so uncertainty was prioritized"
        )

    if strategy["score"] is None:
        return "automatic hit discovery was used with the user-defined objective"
    if strategy["score"] <= 8:
        return f"model score {strategy['score']} is between 7 and 8, so balanced hit discovery was used"
    return f"model score {strategy['score']} is above 8, so a more exploitative hit discovery was used"


def format_score_regime_label(strategy):
    """
    Return a readable label for the score regime.
    """
    score_band = strategy.get("score_band")
    if score_band == "<=6":
        return "score <= 6"
    if score_band == "7-8":
        return "7 <= score <= 8"
    if score_band == ">8":
        return "score > 8"
    if score_band == "manual_model":
        return "manual model mode"
    if score_band == "manual_hit":
        return "manual hit mode"
    return "score unavailable"


def format_score_interpretation(score):
    """
    Return a user-facing interpretation of the selected ROBERT score.
    """
    if score is None:
        return "score unavailable"
    if score <= 3:
        return "very weak model (0-3)"
    if score <= 6:
        return "weak-to-moderate model (4-6)"
    if score <= 8:
        return "good model (7-8)"
    return "strong model (>8)"


def format_score_explanation(strategy):
    """
    Explain what the selected score means for the current AL decision.
    """
    score = strategy.get("score")
    if strategy["score_source"] == "manual_override":
        if strategy["strategy"] == "model":
            return (
                "The score is shown as model context only. The user explicitly requested "
                "uncertainty-based model-improvement mode, so ALMOS prioritizes candidates "
                "that are expected to improve the model the most after experimental evaluation."
            )
        return (
            "The score is shown as model context only. The user explicitly requested "
            "hit-focused selection, so ALMOS ranks candidates using the selected objective "
            "and alpha rather than choosing the strategy automatically from the score."
        )

    if score is None:
        return (
            "The model score could not be read, so ALMOS falls back to the safest learning-first "
            "behavior and prioritizes uncertain candidates."
        )
    if score <= 3:
        return (
            "The model is still far from reliable, so the safest strategy is to focus on learning. "
            "ALMOS prioritizes highly uncertain candidates because they are expected to improve the "
            "model the most after experimental evaluation."
        )
    if score <= 6:
        return (
            "The model is starting to capture useful patterns, but its predictions are still not "
            "strong enough for confident hit-focused selection. ALMOS still prioritizes uncertain "
            "candidates to strengthen the model before shifting toward more exploitative decisions."
        )
    if score <= 8:
        return (
            "The model is now reliable enough to support a balanced search for promising candidates. "
            "ALMOS combines predicted performance with predictive uncertainty so selection can both "
            "discover hits and keep improving the model."
        )
    return (
        "The model is performing well enough to place more trust in its predictions. ALMOS therefore "
        "gives more weight to promising candidates, while still keeping a smaller uncertainty "
        "contribution to avoid overconfidence."
    )


def build_selected_candidates_preview(df, name_column, predictions_column, sd_column):
    """
    Build a readable candidate preview table for the AL log.
    """
    preview_df = df[[name_column, predictions_column, sd_column, "_ranking_metric"]].copy()
    preview_df.insert(0, "rank", range(1, len(preview_df) + 1))
    preview_df = preview_df.rename(
        columns={
            name_column: "candidate",
            predictions_column: "prediction",
            sd_column: "uncertainty",
            "_ranking_metric": "ranking_score",
        }
    )
    numeric_columns = ["prediction", "uncertainty", "ranking_score"]
    for column in numeric_columns:
        preview_df[column] = preview_df[column].map(lambda value: round(float(value), 4))
    return preview_df


def format_text_table(df, max_widths=None):
    """
    Render a DataFrame as a readable plain-text table for .dat logs.
    """
    if df.empty:
        return "(no rows)"
    ...
    for column in rendered_df.columns:
        rendered_df[column] = rendered_df[column].map(
            lambda value: "" if pd.isna(value) else str(value)
        )
        if column in max_widths:
            limit = int(max_widths[column])
            rendered_df[column] = rendered_df[column].map(
                lambda value: value if len(value) <= limit else value[: limit - 3] + "..."
            )
    ...
    for column in rendered_df.columns:
        widths[column] = max(
            len(str(column)),
            rendered_df[column].map(len).max(),
        )

    def build_separator():
        return "+" + "+".join("-" * (widths[column] + 2) for column in rendered_df.columns) + "+"

    def build_row(values):
        cells = []
        for column, value in zip(rendered_df.columns, values):
            if column == "candidate":
                cells.append(f" {value.ljust(widths[column])} ")
            else:
                cells.append(f" {value.rjust(widths[column])} ")
        return "|" + "|".join(cells) + "|"

    separator = build_separator()
    lines = [
        separator,
        build_row(rendered_df.columns),
        separator,
    ]
...
def describe_metric_transition(metric_name, previous_value, current_value, tolerance, converged):
    """
    Create a readable one-line summary for convergence checks.
    """
    if previous_value == 0:
        relative_change = 0.0
    else:
        relative_change = abs(previous_value - current_value) / abs(previous_value) * 100
    ...
    if metric_name == "score":
        ...
        return (
            f"{metric_name}: {previous_value} -> {current_value} | "
            f"{movement} | {status}"
        )
    ...
    status = "converged" if converged else "not converged"
    return (
        f"{metric_name}: {previous_value:.4f} -> {current_value:.4f} | "
        f"{movement} | change = {relative_change:.2f}% | "
        f"tolerance = {tolerance * 100:.2f}% | {status}"
    )
...
def check_missing_outputs(self):
        ...
        - Validates acquisition settings such as objective, mode, and alpha.
        - Validates 'tolerance' ensuring valid ranges.
        ...
        # Normalize metadata columns used by AL before validating the dataset.
        # Batch is metadata, not a descriptor, regardless of capitalization.
        batch_columns = [
            column for column in self.df_raw.columns
            if column.lower() == str(self.batch_column).lower()
        ]
        if len(batch_columns) > 1:
            print(
                f"\nx WARNING! Multiple batch columns were found with different "
                f"capitalization: {batch_columns}. Please keep only one. Exiting."
            )
            sys.exit()
        if batch_columns and batch_columns[0] != self.batch_column:
            self.df_raw = self.df_raw.rename(columns={batch_columns[0]: self.batch_column})
            print(f"\no Batch column '{batch_columns[0]}' renamed to '{self.batch_column}'.")

        for column in self.df_raw.columns:
            if column.lower() == "smiles" and column not in self.ignore:
                self.ignore.append(column)
        ...
        if self.y not in self.df_raw.columns:
            ...
            sys.exit()
        valid_y_mask = self.df_raw[self.y].notna() & (
            self.df_raw[self.y].astype(str).str.strip() != ""
        )

        # Validate active-learning objective and acquisition settings
        if self.al_mode is not None:
            self.al_mode = str(self.al_mode).strip().lower()
            if self.al_mode == "":
                self.al_mode = None
        if self.al_mode not in {None, "model", "hit"}:
            print(
                f"\nx WARNING! The mode '{self.al_mode}' is not valid. "
                "Use '--mode model' or '--mode hit'. Exiting."
            )
            sys.exit()
        ...
        if self.alpha is not None:
            try:
                ...
                print(
                    f"\nx WARNING! Invalid alpha value '{self.alpha}'. "
                    "Expected a number between 0 and 1. Exiting."
                )
                sys.exit()
        ...
        if self.objective is not None and str(self.objective).strip() != "":
            self.objective = str(self.objective).strip().lower()
            if self.objective not in {"max", "min"}:
                print(
                    f"\nx WARNING! The objective '{self.objective}' is not valid. "
                    "Use '--objective max' or '--objective min'. Exiting."
                )
                sys.exit()
        else:
            self.objective = None

        if self.al_mode != "model" and self.objective is None:
            print(
                "\nx WARNING! The objective must be specified with "
                "'--objective max' or '--objective min' unless '--mode model' is used. Exiting."
            )
            sys.exit()

        if self.al_mode == "model" and self.alpha is not None:
            print(
                "\nx WARNING! '--alpha/--alfa' cannot be used with '--mode model' "
                "because model mode ranks only by uncertainty. Exiting."
            )
            sys.exit()
        ...
        if self.batch_column in self.df_raw.columns:
            batch_values = pd.to_numeric(self.df_raw[self.batch_column], errors="coerce")
            self.df_raw[self.batch_column] = batch_values
            if batch_values.notna().any():
                max_batch_number = int(batch_values.max())
            elif valid_y_mask.any():
                self.df_raw.loc[valid_y_mask, self.batch_column] = 0
                self.df_raw.to_csv(self.path_csv_name, index=False)
                max_batch_number = 0
                print(
                    f"\no Batch column '{self.batch_column}' had no valid values; "
                    f"assigned batch 0 to rows with values in '{self.y}'."
                )
            else:
                print(
                    f"\nx WARNING! '{self.batch_column}' column has no valid values, "
                    f"and '{self.y}' has no values! Exiting."
                )
                sys.exit()
            ...
            if not self.df_raw[valid_y_mask & self.df_raw[self.batch_column].isna()].empty:
            ...
            if self.y in self.df_raw.columns and valid_y_mask.any():
                self.df_raw[self.batch_column] = None
                self.df_raw.loc[valid_y_mask, self.batch_column] = 0
                self.df_raw.to_csv(self.path_csv_name, index=False)
        ...
        # Add batch column to ignore list
...
def resolve_active_learning_strategy(score, objective, mode=None, alpha_override=None):
    """
    Resolve the ranking strategy for active learning.

    Parameters:
    -----------
    score : int or float or None
        Score of the prediction model selected for ranking.
    objective : str
        Either 'max' or 'min'.
    mode : str or None
        Optional manual mode override: 'model' or 'hit'.
    alpha_override : float or None
        Optional alpha supplied by the user.

    Returns:
    --------
    dict
        Strategy metadata containing the resolved mode and ranking settings.
    """
    objective = None if objective is None else str(objective).lower()
    if mode == "model":
        return {
            "strategy": "model",
            "objective": objective,
    ...
    if mode == "hit":
        alpha_value = 1.0 if alpha_override is None else float(alpha_override)
        return {
            "strategy": "hit",
            "objective": objective,
            "alpha": alpha_value,
            "score": score,
            "score_band": "manual_hit",
            "score_source": "manual_override",
        }
    ...
    if score is None or score <= 6:
        return {
            ...
            "alpha": None,
            "score": score,
            "score_band": "<=6" if score is not None else "unknown",
            "score_source": "auto",
        }
    ...
    return {
        ...
        "alpha": alpha_value,
        "score": score,
        "score_band": "7-8" if score <= 8 else ">8",
        "score_source": "auto",
    }
...
def rank_active_learning_candidates(
    ...
    """
    Rank candidate rows for active learning using uncertainty-only or hit discovery.

    Parameters:
    -----------
    df : pd.DataFrame
        Candidate rows without assigned batch values.
    strategy : dict
        Strategy metadata returned by resolve_active_learning_strategy.
    predictions_column : str
        Column with model predictions.
    sd_column : str
        Column with predictive uncertainty.
    selection_size : int or None
        Intended number of candidates to select in the current batch.

    Returns:
    --------
    pd.DataFrame
        Ranked copy of the candidate DataFrame including acquisition metadata columns.
    """
    ranked_df = df.copy()
    ranked_df["_pred_value"] = ranked_df[predictions_column]
    ranked_df["_uncertainty_value"] = ranked_df[sd_column]

    if strategy["strategy"] == "model":
        ranked_df["_selection_penalty"] = 0.0
        ranked_df["_ranking_metric"] = ranked_df[sd_column].astype(float)
        ranked_df["_acquisition_label"] = "uncertainty"
        ranked_df = ranked_df.sort_values(
            by=["_ranking_metric", predictions_column],
            ascending=[False, False],
        )
        return ranked_df

    alpha = float(strategy["alpha"])
    if strategy["objective"] == "max":
        ranked_df["_ranking_metric"] = ranked_df[predictions_column] + alpha * ranked_df[sd_column]
        ranked_df["_acquisition_label"] = "prediction + alpha*uncertainty"
        ranked_df = ranked_df.sort_values(
            by=["_ranking_metric", predictions_column, sd_column],
...
def _assign_prediction_quartiles(df, predictions_column):
    """
    Assign min-max quartile bins based on predicted target values.
    """
    quartile_df = df.copy()
    min_pred = float(quartile_df[predictions_column].min())
    max_pred = float(quartile_df[predictions_column].max())

    if min_pred == max_pred:
        quartile_df["_prediction_quartile"] = "q1"
        quartile_df["_prediction_quartile_index"] = 0
        quartile_df["_quartile_range"] = f"[{min_pred:.4f}, {max_pred:.4f}]"
        quartile_df.attrs["prediction_quartile_bounds"] = [min_pred] * 5
        return quartile_df

    step = (max_pred - min_pred) / 4.0
    bounds = [min_pred + step * idx for idx in range(5)]
    bounds[-1] = max_pred

    def quartile_index(value):
        if value <= bounds[1]:
            return 0
        if value <= bounds[2]:
            return 1
    ...
    quartile_df["_prediction_quartile_index"] = quartile_df[predictions_column].map(quartile_index)
    quartile_df["_prediction_quartile"] = quartile_df["_prediction_quartile_index"].map(
        lambda idx: f"q{idx + 1}"
    )
    quartile_df["_quartile_range"] = quartile_df["_prediction_quartile_index"].map(
        lambda idx: f"[{bounds[idx]:.4f}, {bounds[idx + 1]:.4f}]"
    )
    quartile_df.attrs["prediction_quartile_bounds"] = bounds
    return quartile_df


def _rank_model_candidates_with_quartile_diversity(
    df,
    predictions_column,
    sd_column,
    selection_size=None,
    ...
    """
    Rank model-improvement candidates by SD with a soft diversity penalty over y_pred quartiles.
    """
    ranked_df = _assign_prediction_quartiles(df, predictions_column)
    ranked_df["_sd_base"] = ranked_df[sd_column].astype(float)
    ranked_df["_selection_penalty"] = 0.0
    ranked_df["_ranking_metric"] = ranked_df["_sd_base"]
    ranked_df["_selection_rank"] = 0
    ranked_df["_acquisition_label"] = "uncertainty - quartile overrepresentation penalty"

    selected_indices = []
    quartile_counts = {idx: 0 for idx in range(4)}
    if selection_size is None:
        effective_selection_size = len(ranked_df)
    else:
        effective_selection_size = min(max(int(selection_size), 1), len(ranked_df))
    target_per_quartile = max(effective_selection_size / 4.0, 1.0)
    lambda_penalty = float(ranked_df["_sd_base"].max()) * 0.15

    remaining_df = ranked_df.copy()
    rank_position = 1

    while not remaining_df.empty:
        remaining_with_penalty = remaining_df.copy()
        remaining_with_penalty["_selection_penalty"] = remaining_with_penalty[
            "_prediction_quartile_index"
        ].map(lambda idx: lambda_penalty * (quartile_counts[idx] / target_per_quartile))
        remaining_with_penalty["_ranking_metric"] = (
            remaining_with_penalty["_sd_base"] - remaining_with_penalty["_selection_penalty"]
        )

        selected_row = remaining_with_penalty.sort_values(
            by=["_ranking_metric", "_sd_base", predictions_column],
            ascending=[False, False, False],
        ).iloc[0]

        selected_index = selected_row.name
        ranked_df.at[selected_index, "_selection_penalty"] = float(selected_row["_selection_penalty"])
        ranked_df.at[selected_index, "_ranking_metric"] = float(selected_row["_ranking_metric"])
        ranked_df.at[selected_index, "_selection_rank"] = rank_position

        selected_indices.append(selected_index)
        quartile_counts[int(selected_row["_prediction_quartile_index"])] += 1
        remaining_df = remaining_df.drop(index=selected_index)
        rank_position += 1

    ranked_df = ranked_df.loc[selected_indices].copy()
    ranked_df.attrs["model_diversity_lambda"] = lambda_penalty
    ranked_df.attrs["model_diversity_target_per_quartile"] = target_per_quartile
    ranked_df.attrs["model_diversity_quartile_counts"] = {
        f"q{idx + 1}": quartile_counts[idx] for idx in range(4)
    }
    return ranked_df
...
def get_metrics_from_batches():
    ...
    for batch_dir in Path.cwd().glob('batch_*'):
        ...
        batch_suffix = batch_dir.name.split('_', 1)[1] if '_' in batch_dir.name else ""
        if not batch_suffix.isdigit():
            continue
        if batch_suffix == '0':
            continue

        no_pfi_result, pfi_result = process_batch(batch_suffix)
...
class EarlyStopping:
    ...
    def show_summary(self, df, model_type):
        ...
        write_log_block(
            self.log,
            f"{model_type} convergence summary",
            [
                ("Total iterations", len(batch)),
                ("Final score", f"{scores[-1]} (started at {scores[0]})"),
                ("Final RMSE", f"{rmse[-1]:.2f} (started at {rmse[0]:.2f})"),
                ("Final SD", f"{sd[-1]:.2f} (started at {sd[0]:.2f})"),
                (
                    "Interpretation",
                    "model behavior has stabilized and large improvements are no longer expected",
                ),
            ],
        )
    ...
    def check_convergence_model(self, df, model_type):
        ...
        """
        write_log_header(self.log, f"{model_type} Convergence Review")
        ...
        if df.shape[0] < 2:
            ...
            write_log_block(
                ...
                [
                    ("Model", model_type),
                    ("Batches available", df.shape[0]),
                    ("Decision", "not enough batches to evaluate convergence yet"),
                ],
            )
        ...
        for i in range(1, min(self.patience + 1, df.shape[0])):
            ...
            write_log_block(
                self.log,
                f"Batch {int(current_row['batch'])} vs batch {int(previous_row['batch'])}",
                [
                    (
                        "Score",
                        describe_metric_transition(
                            "score",
                            previous_row[score_column],
                            current_row[score_column],
                            self.score_tolerance,
                            patience_convergence["score"],
                        ),
                    ),
                    (
                        "RMSE",
                        describe_metric_transition(
                            "rmse",
                            previous_row[rmse_column],
                            current_row[rmse_column],
                            self.rmse_min_delta,
                            patience_convergence["rmse"],
                        ),
                    ),
                    (
                        "SD",
                        describe_metric_transition(
                            "sd",
                            previous_row[sd_column],
                            current_row[sd_column],
                            self.sd_min_delta,
                            patience_convergence["SD"],
                        ),
                    ),
                ],
            )
        ...
        if score_no_improvement_streak >= self.patience:
            ...
            write_log_block(
                ...
                "Recommendation",
                [
                    ("Decision", "consider stopping this model"),
                    (
                        "Reason",
                        f"the score has not improved for {score_no_improvement_streak} consecutive batches",
                    ),
                ],
            )  
        ...
        if score_value_pfi is not None and score_value_pfi >= 8:
            ...
            write_log_block(
                ...
                "Recommendation",
                [
                    ("Decision", "PFI may be ready to stop"),
                    ("Reason", f"latest PFI score is already very good ({score_value_pfi:.2f})"),
                ],
            )
        ...
        if score_value_no_pfi is not None and score_value_no_pfi >= 8:
            ...
            write_log_block(
                ...
                "Recommendation",
                [
                    ("Decision", "No_PFI may be ready to stop"),
                    ("Reason", f"latest No_PFI score is already very good ({score_value_no_pfi:.2f})"),
                ],
            )
        ...
        if no_improvement_streak >= self.patience:
            ...
            write_log_block(
                ...
                [
                    ("Status", "converged"),
                    ("Interpretation", "additional active learning rounds are unlikely to improve this model substantially"),
                ],
            )
            stop_recommended = True
        ...
        if not stop_recommended:
            write_log_block(
                ...
                "Final decision",
                [
                    ("Status", "not converged yet"),
                    ("Interpretation", "additional active learning rounds may still improve this model"),
                ],
            )
